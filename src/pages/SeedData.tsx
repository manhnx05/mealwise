  ...
  const handleSeed = async () => {
    ...
    for (const recipe of sampleRecipes) {
      try {
        ...
        newResults.push({ type: 'recipe', title: recipe.title, success: true });
      } catch (error: any) {
        newResults.push({ type: 'recipe', title: recipe.title, success: false, error: error.message });
      }
    ...
    for (const post of sampleCommunityPosts) {
      try {
        ...
        newResults.push({ type: 'post', title: post.title, success: true });
      } catch (error: any) {
        newResults.push({ type: 'post', title: post.title, success: false, error: error.message });
      }
      setResults([...newResults]);
    }
      ...
      <Card className="p-6 mb-6">
        <div className="flex items-center gap-4 mb-4">
          ...
          <div className="flex-1">
            <h3 className="font-semibold text-lg">Dữ liệu sẽ được tạo:</h3>
            <p className="text-sm text-muted-foreground">
              {sampleRecipes.length} công thức nấu ăn + {sampleCommunityPosts.length} bài viết cộng đồng
            </p>
          </div>
        </div>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {result.type === 'recipe' ? 'Công thức' : 'Bài viết'}
                  </p>
